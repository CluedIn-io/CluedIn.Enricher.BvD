...
namespace CluedIn.Provider.BvD
{
    public class BvDSearchProviderProvider : ProviderBase, IExtendedProviderMetadata, IExternalSearchProviderProvider
    {
        ...
        public BvDSearchProviderProvider([System.Diagnostics.CodeAnalysis.NotNull] ApplicationContext appContext) : base(appContext, GetMetaData())
        {
        ...
        public override async Task<IDictionary<string, object>> GetHelperConfiguration(ProviderUpdateContext context, CrawlJobData jobData, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            ...
            throw new InvalidOperationException($"Unexpected data type for {nameof(BvDExternalSearchJobData)}, {jobData.GetType()}");
        }
